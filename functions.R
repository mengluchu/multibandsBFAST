...
# library(knitr) library(bfastSpatial)
library(plyr)
library(bfast)
library(raster)
library(lubridate)
library(devtools)
# install_github('edzer/spacetime') library(spacetime)

# library(devtools) install_github('dutri001/bfastSpatial')
...
bfmPCAALL <- function(arr7bands2, history = c("all", "ROC", "BP"), hisweight = T, 
    timearr, pcacomp, moy = 1, scoreselect = F, lastordetect = c("last", "detect"), 
    sca = F, plot = F) {
    ...
    # timemulbands<-timendmi[-as.numeric(attr(na.omit(t(arr7bands2[,1,])),
    # 'na.action'))]
    breakpointx <- NA
    
    for (i in 1:dim(a7bandsrm2)[2]) {
        ta7b <- t(a7bandsrm2[, i, ])
        bfmic <- try(ybfastmonitorPCA(arr = ta7b, hisweight = hisweight, history = history, 
            myear = 2005, my_dates = timearr, moy = moy, plot = plot, lastordetect = lastordetect, 
...
bfmndmiALL <- function(arr7bands1, timearr, lastordetect = c("last", "detect"), history = c("all", 
    "ROC", "BP"), plot = FALSE, moy = 1) {
    ...
    for (i in 1:dim(a7bandsrm2)[2]) {
        ta7b <- arr7bands1[i, ]
        bfmic <- try(ybfastmonitorndmi(x = ta7b, myear = 2005, history = history, 
            my_dates = timearr, moy = moy, plot = plot), silent = TRUE)
        if (class(bfmic) != "try-error") {
            if (!is.na(bfmic)) 
                timebfm[i] <- bfmic
        }
    ...
    # plot(timedif3,main='ndmi',typ='h', ylab='time differences compare with
    # validation data') return(timedif3)
...
ybfastmonitorPCA <- function(arr, hisweight, pcacomp, myear, plot = F, history = c("all"), 
    my_dates, scoreselect = F, lastordetect = c("last", "detect"), minumum_observations = 15, 
    sca, type = "OLS-MOSUM", moy = 1) {
    ...
    if (length(densetime) > minumum_observations) {
        ...
        if (length(historyPeriod2) > 6) {
            ...
            while (is.na(breakpointx) & coun < length(timex)) {
                ...
                stmon2 <- atimex[acoun]
                ## this is for one value dtat2 <- subset(dtat, round (dtat$my_date , digits = 6) <
                ## stmon) # control time length
                ...
                ## PCA came in
                if (hisweight) {
                  PCAhis <- omarr[(round(decimal_date(ondtime), digits = 6) < stmon2), 
                ...
                bfm <- bfastmonitor(data = bfts, formula = response ~ 1, start = c(stmon2, 
                  1), plot = plot, history = history, type = type)
...
ybfastmonitorndmi <- function(x, moy = 1, myear = 2005, plot = F, pCentile = 0.05, 
    history, my_dates, minumum_observations = 15, type = "OLS-MOSUM") {
        ...
        if (length(historyPeriod2$dates) > 6) {
            ...
            while (is.na(breakpointx) & coun < length(timex)) {
                ...
                bfm <- bfastmonitor(data = bpts, order = 1, formula = response ~ 
                  harmon, start = c(stmon, 1), plot = plot, history = history, type = type)
                ...
                if (!is.na(breakpoint1)) {
                  ...
                  cdte <- subset(dtat2, round(dtat2$my_date, digits = 2) >= round(tbreak, 
                    digits = 2) & round(dtat2$my_date, digits = 3) <= round(xbreak, 
                    digits = 3))
...
# array data without change bolivia load timearr
...
########################################### Bochange<-Brazilchange # brazil Bochange<-Bchange_1000 # bolivia

# Bo5<-Bochange[ ,grep('sr_band5',x=colnames(Bochange))] Bo4<-Bochange[
# ,grep('sr_band4',x=colnames(Bochange))] Bo1<-Bochange[
# ,grep('sr_band1',x=colnames(Bochange))] Bo2<-Bochange[
# ,grep('sr_band2',x=colnames(Bochange))] Bo3<-Bochange[
# ,grep('sr_band3',x=colnames(Bochange))] Bo7<-Bochange[
# ,grep('sr_band7',x=colnames(Bochange))]

# lc<-dim(Bochange)[1]

# arr6bands<-array(NA,c(6,lc,ncol(Bo5))) arr6bands[1,,]<-Bo1 arr6bands[2,,]<-Bo2
# arr6bands[3,,]<-Bo3 arr6bands[4,,]<-Bo4

# arr6bands[5,,]<-Bo5 arr6bands[6,,]<-Bo7

# ndmichange<-(as.matrix(Bo4)-as.matrix(Bo5))/(as.matrix(Bo4)+as.matrix(Bo5))
# ndvichange<-(as.matrix(Bo4)-as.matrix(Bo3))/(as.matrix(Bo3)+as.matrix(Bo4)) #
# try ndvi

######################################################## 

t2t3ns <- bfmPCAALL(arr7bands2 = Brazilarr, hisweight = T, timearr = Braziltime, 
    plot = FALSE, pcacomp = 2, moy = 1, lastordetect = "last", history = "all")
nt2t3ns <- bfmPCAALL(arr7bands2 = Brazilarrno, hisweight = T, timearr = Braziltime, 
    plot = FALSE, pcacomp = 2, moy = 1, lastordetect = "last")

# put the results of two arrays together
wrap <- function(arr7, arr7no, plot = F, pcacomp, moy, lastordetect = "last", wholet, 
    history = "all", hisweight = T, sca, scoreselect = F) {
    t1 <- bfmPCAALL(arr7bands2 = arr7, hisweight = hisweight, sca = sca, timearr = wholet, 
        plot = plot, pcacomp = pcacomp, moy = moy, lastordetect = lastordetect, history = history, 
        scoreselect = scoreselect)
    
    nt1 <- bfmPCAALL(arr7bands2 = arr7no, hisweight = hisweight, sca = sca, timearr = wholet, 
        plot = plot, pcacomp = pcacomp, moy = moy, lastordetect = lastordetect, history = history, 
        scoreselect = scoreselect)
    tn <- c(t1, nt1)
    return(tn)
...
brhisautos <- wrap(arr7 = Brazilarr, arr7no = Brazilarrno, wholet = Braziltime, hisweight = T, 
    moy = 1, sca = T)

brscore3s <- wrap(arr7 = Brazilarr, arr7no = Brazilarrno, pcacomp = 3, wholet = Braziltime, 
    hisweight = F, moy = 1, sca = T)

brscore2s <- wrap(arr7 = Brazilarr, arr7no = Brazilarrno, pcacomp = 2, wholet = Braziltime, 
    hisweight = F, moy = 1, sca = T)

brscore1s <- wrap(arr7 = Brazilarr, arr7no = Brazilarrno, pcacomp = 1, wholet = Braziltime, 
    hisweight = F, moy = 1, sca = T)

brscoreautos <- wrap(arr7 = Brazilarr, arr7no = Brazilarrno, wholet = Braziltime, 
    hisweight = F, scoreselect = T, moy = 1, sca = T)




bohisautunscale <- wrap(arr7 = Boliviaarr, arr7no = Boliviaarrno, wholet = time_B1000, 
    hisweight = T, moy = 1, sca = T)

boscore3scale <- wrap(arr7 = Boliviaarr, arr7no = Boliviaarrno, pcacomp = 3, wholet = time_B1000, 
    hisweight = F, moy = 1, sca = T)

boscore2scale <- wrap(arr7 = Boliviaarr, arr7no = Boliviaarrno, pcacomp = 2, wholet = time_B1000, 
    hisweight = F, moy = 1, sca = T)

boscore1scale <- wrap(arr7 = Boliviaarr, arr7no = Boliviaarrno, pcacomp = 1, wholet = time_B1000, 
    hisweight = F, moy = 1, sca = T)

boscoreautoscale <- wrap(arr7 = Boliviaarr, arr7no = Boliviaarrno, wholet = time_B1000, 
    hisweight = F, scoreselect = T, moy = 1, sca = T)
...
str(valichart)
# vctest<-read.csv('VCtest.csv') vctest[,'ChangeDate']<-
valichart <- data.frame(cbind(BTest$ChangeDate, band6o1, band7o1, PCA1, PCA2, PCA3, 
    ndmi, ndvi))
...
# save(valichart, file='valichart.Rdata') save(valichartBrazil2,
# file='valichartBrazil2.Rdata')

############### TCT ############################################################### arr:
############### spectral, spatial, time l1: length of landsat tm5 236 for bolivia, 120 for
############### brazil
...
TBbr <- bfmndmiALL(arr7bands1 = tctbr[[1]], results = "time", timearr = Braziltime, 
    moy = 1, plot = FALSE)
TGbr <- bfmndmiALL(arr7bands1 = tctbr[[2]], results = "time", timenarr = Braziltime, 
    moy = 1, plot = FALSE)
TWbr <- bfmndmiALL(arr7bands1 = tctbr[[3]], results = "time", timenarr = Braziltime, 
    moy = 1, plot = FALSE)

TBbrno <- bfmndmiALL(arr7bands1 = tctbrno[[1]], results = "time", timearr = Braziltime, 
    moy = 1, plot = FALSE)
TGbrno <- bfmndmiALL(arr7bands1 = tctbrno[[2]], results = "time", timearr = Braziltime, 
    moy = 1, plot = FALSE)
TWbrno <- bfmndmiALL(arr7bands1 = tctbrno[[3]], results = "time", timearr = Braziltime, 
    moy = 1, plot = FALSE)
...
######################## animation ############## saveGIF({ ani.options(nmax = 30)
######################## ybfastmonitorPCA(arr=t(arr7bands[,11,]),myear=2005,my_dates=timendmi1,moy=1,lastordetect='detect',plot=FALSE)
######################## ybfastmonitorndmi(x=
######################## ndmichange[9,],myear=2005,my_dates=timendmi,moy=2,plot=TRUE) }, interval = 0.1,
######################## movie.name = 'bfmndmi.gif', ani.width = 600, ani.height = 600)
