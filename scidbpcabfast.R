...
iquery("r_exec(crsubboli4, 'output_attrs=1',
       'expr= 
sink(paste(\"/home/menglu/\",the_i[1],the_j[1],\".txt\" ))
  ...
  arr6bands <- array(NA,c(6,dim3, dim2, dim1))
      ...
      newarray  <- try(aperm (arr6bands, c(1,4,3,2)))
      newarr    <- try(rearrange_array (newarray, flatten=c(2,3)))
       timearrbo <- try(sort(time_B1000))      
 ...
 a1 <- try(bfmPCA(multibandsarr= newarr, timearr=timearrbo, history=\"all\",
      ...
      # {
      # a1=array(rep(-1.0, dim1*dim2), c(dim1, dim2))
      # }  
...
a1<-as.vector(a1)
       ...
       rcol=the_i
       rrow=the_j
       a2 <- as.double(as.vector(a1))
print(a1)
       list(rcol, rrow, a2)')"
       ,return=T)
   

iquery("store(r_exec(crsubboli2, 'output_attrs=3',
       'expr=
       sink(paste(\"/home/menglu/\",the_i[1],the_j[1],\".txt\" ))
      ...
      newarr    <- rearrange_array (newarray, dim=c(6, dim1*dim2, dim3))
...
#library(scidb)
 ...
 iquery("store(subarray(LANDSAT_BOLIVIA,
   0,0,0,3,3,1500), subbolivia)")
 iquery("store(apply(repart(subbolivia,
<band1:double,band2:double,band3:double,band4:double,band5:double,band6:double> [y=0:3,2,0,x=0:3,2,0,t=0:1500,1500,0]), the_i,double(x)
         ,the_j,double(y),the_t,double(t)), crsubboli4)")
