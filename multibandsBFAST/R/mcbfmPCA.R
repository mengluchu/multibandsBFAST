#' @title sequentially Monitor change with BFAST-PCA, use multicore 
...
#' @import plyr
#' @export 
mcbfmPCA <- function(multibandsarr, history = c("all", "ROC", "BP"), hisweight, timearr, 
                   pcacomp, moy = 1, myear = 2005, scoreselect, lastordetect = c("last", "detect"), 
                   sca = F, mc.cores=1) {
  ...
  bfmic <- try(mcapply(a7bandsrm2, 2, pca_bfm, mc.cores=mc.cores, hisweight = hisweight, history = history, 
