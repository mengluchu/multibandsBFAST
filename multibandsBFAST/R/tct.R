...
tct<-function(arr, l1)
  ...
  tasselcap2<-function (arr, sat ) 
  {
    ...
    coefs <- list(landsat4tm = matrix(c(0.2043, 0.4158, 0.5524, 
                                        ...
                                        -0.6109), ncol = 3, dimnames = d), landsat5tm = matrix(c(0.2043, 
                                                                                                 0.4158, 0.5524, 0.5741, 0.3124, 0.2303, 0.1063, -0.2819, 
                                                                                                 -0.4934, 0.794, -2e-04, -0.1446, 0.0315, 0.2021, 0.3102, 
                                                                                                 0.1594, -0.6806, -0.6109), ncol = 3, dimnames = d), 
                  landsat7etm = matrix(c(0.3561, 0.3972, 0.3904, 0.6966, 
                                         0.2286, 0.1596, -0.3344, -0.3544, -0.4556, 0.6966, 
                                         -0.0242, -0.263, 0.2626, 0.2141, 0.0926, 0.0656, 
                                         -0.7629, -0.5388), ncol = 3, dimnames = d), landsat8oli = matrix(c(0.3029, 
                                                                                                            0.2786, 0.4733, 0.5599, 0.508, 0.1872, -0.2941, 
                                                                                                            -0.243, -0.5424, 0.7276, 0.0713, -0.1608, 0.1511, 
                                                                                                            0.1973, 0.3283, 0.3407, -0.7117, -0.4559), ncol = 3, 
  ...
  for( i in 1:dim(arr)[2])
  {
    ...
    tas<-as.matrix(data.frame(cbind(arr[1,i,],arr[2,i,],
