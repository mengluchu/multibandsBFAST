...
#' @return dataframe with confusion matrix and time delay. can use stargazer to generate a table
#' @note adapted from Eliakim's code
#' @export 
# VALIDATION
# **************************************************************************
# require(stargazer) set working directory
...
# Bolivia bovali<-valitable(cx2=valichart,oridensetime=time_B1000,
# oritemplate= bobo5)

# Brazil brvali<-valitable(valichartBrazil2, Braziltime, brbo5)
# stargazer(bovali, summary =FALSE)

valitable <- function(cx2, oridensetime, oritemplate, EarlyDateIsCommission = T, 
    totalp, nofchange, colmWith = 2) {
    
    spatialAccurayAssessmentx <- function(x, nOfindices, EarlyDateIsCommission, 
        TotalSamplesize, snumberOfsamplefromChange, colmWith = 2) {
        # require(lubridate)
        ...
        for (i in 1:pa4) {
            ...
            for (j in 1:length(cp1)) {
                ...
                tr0 <- xt2 + 0.002
                if (tl0 == T & !is.na(xt1) & !is.na(xt2) & xt1 <= tr0 | 
                  tl0 == T & !is.na(xt1) & !is.na(xt2) & xt1 > tr0 & EarlyDateIsCommission == 
                    F) {
                  ba0 <- "tp"
                  va0 <- c(va0, ba0)
                  et3 <- F
                ...
                if (tl0 == T & et3 == T & be1 == T & !is.na(xt1) & !is.na(xt2) | 
                  tl0 == T & et3 == T & be1 == T & !is.na(xt1) & !is.na(xt2) & 
                    xt1 > xt2 & EarlyDateIsCommission == T) {
                  ba0 <- "fp"
                  va0 <- c(va0, ba0)
                  ze0 <- F
                } else {
            ...
            # calculate overall accuracy and errors co <- (length(subset (va0 , va0
            # == 'co'))/ length(va0))*100 ok <- (length(subset (va0 , va0 == 'ok'))/
            # length(va0))*100 om <- (length(subset (va0 , va0 == 'om'))/
            # length(va0))*100 x0 <- c(ok, co, om) xo <- rbind(xo,x0)
            
            # number of true
            
            uSA <- (length(subset(va0, va0 == "tp")))/(length(subset(va0, 
                va0 == "tp")) + length(subset(va0, va0 == "fp"))) * 100
            pPA <- (length(subset(va0, va0 == "tp")))/(length(subset(va0, 
                va0 == "tp")) + length(subset(va0, va0 == "fn"))) * 100
            po <- length(subset(va0, va0 == "tp"))/(length(subset(va0, va0 == 
                "tp")) + length(subset(va0, va0 == "fn")) + length(subset(va0, 
                va0 == "fp"))) * 100
            dox <- c(round(po, digit = 1), round(pPA, digit = 1), round(uSA, 
                digit = 1))
            xo <- rbind(xo, dox)
        }
    ...
    temporalDelay <- function(vdata, dtes, TmSeriesOfSamplePixels, nOfindices = 1) {
