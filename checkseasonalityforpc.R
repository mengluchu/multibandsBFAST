...
# inputarr: input array: Boliviaarrno: dim(Boliviaarrno) 6 1033 444 timearr: the
# time of the imagery series. 444 loca: spatial index monitoryear: before the
# monitor year is the historical period
returnpc2 <- function(inputarr, timearr, loca, preprocess = T, monitoryear) {
    ...
    pcacompauto <- which.max(abs(apply(fit$rotation[1:3, ] - fit$rotation[4:6, ], 
        2, sum)))
    pcacompauto2 <- which.max(abs(fit$rotation[4, 2:3] - fit$rotation[6, 2:3])) + 
        1
...
# return 3 variables: 1. the removed na index. 2 Historical PC score, 3 pc
# greeness score
checkseats <- function(ts, order, time1) {
    ...
    zz <- zoo(na.omit(ts), 1900 + as.POSIXlt(time1)$year + (yday365(time1) - 1)/365, 
        frequency = 365)
    # zz<-zz[ time(zz)>'2000(023)'&time(zz)<'2015(023)'] zz <- aggregate(zz,
    # as.yearmon, mean)
...
# if preprocess, need the library(multibandsBFAST) Boliviaarrno<-
# aaply(Boliviaarrno,c(1,2),rmsat) #remove extreme value outside valid range
# (1-10000) Boliviaarrno<-aaply(Boliviaarrno,c(1,2),removedips) # remove low
# value
